...
- 2026-07-09 — Discovery (`sonar`) returned malformed JSON ~half the time (clean array vs double-encoded string vs prose with no array), so `extractJson` 500'd or returned empty. FINAL fix = Perplexity **structured output**: pass `response_format:{type:'json_schema', json_schema:{schema:{type:'object', properties:{sources:{type:'array',items:{type:'string'}}}, required:['sources']}}}` (threaded through `run()` → `callOpenAICompatible` as `responseFormat`), which forces valid JSON on one ~5s call (verified 8/8). `discoverSources()` keeps the best of up to 3 attempts (early-return once ≥3 clean sources) purely to guard the rare degenerate run; `parseSourceArray()` pulls the array from the `{sources}` object and stays tolerant of the old shapes. NOTE: an interim SEQUENTIAL 4-retry loop worked but made discovery take up to ~16s (Jordan noticed the regression) — structured output got it back to a single call. Anthropic caller ignores `responseFormat`. Schema doesn't cap count, so sonar occasionally over-produces (e.g. 28) — editable chips, acceptable.
- 2026-07-09 — Brand (wordmark + URL) edited in Creative Studio now persists to its own localStorage key (`meta-ads-brand`), initialised on mount + saved on change (`CreativeStudio.jsx`). Why: the studio unmounts when you leave the Creatives tab, so brand edits reset on return. Deliberately a SEPARATE key from the per-run `pipeline-state-v1` so a brand survives "Start over" (a brand doesn't change per product). Palette + template-selection still reset on remount (not persisted) — same one-line fix if it's ever raised. Both `CreativeStudio.jsx` copies (this repo + `pricing-landing-page/src/meta-ads/`) edited identically.
